  ...
  $('button').on('click', function () {
    ...
    $.ajax({
      ...
      $('.showtoggle').on('click', function () {
        if ($(this).text() === 'Show') {
          $(this).text('Hide');
          const placeId = $(this).attr('place_id');
          const ulelem = $(this).closest('.reviews').find('ul');
          ulelem.empty();
          $.get(`http://localhost:5001/api/v1/places/${placeId}/reviews`, function (data) {
